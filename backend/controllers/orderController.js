...
exports.confirmSingleCart = async (req, res) => {
  ...
  if (stock <= 0) return res.status(400).json({ error: "Run out of this product" });
  const { rows: users } = await pool.query(
    'SELECT name, contact, city, address FROM users WHERE email = $1',
    [email]
  );
  const user = users[0];
  await pool.query('UPDATE cart_items SET status = $1 WHERE id = $2', ["waiting", item_id]);
...
exports.confirmOrder = async (req, res) => {
  ...
  const { rows: users } = await pool.query(
    'SELECT name, contact, city, address FROM users WHERE email = $1', [email]
  );
  const user = users[0];
  const { rows: cartItems } = await pool.query(
    `SELECT ci.item_id, i.price, i.name, i.location, i.stock, COUNT(*) AS quantity
     ...
     GROUP BY ci.item_id, i.price, i.name, i.location, i.stock`, [email]
  );
  if (!cartItems.length) return res.status(400).json({ error: "Cart is empty" });

  for (const item of cartItems) {
    const { rows: reservedRows } = await pool.query(
      `SELECT COUNT(*) FROM cart_items WHERE item_id = $1 AND status = 'waiting'`, [item.item_id]
    );
...
exports.getOrders = async (req, res) => {
  ...
  const { rows } = await pool.query(
    `SELECT items.name, items.price, cart_items.id, cart_items.status, cart_items.created_at
     ...
     ORDER BY cart_items.created_at DESC`, [email]
  );
  res.json(rows);
};
